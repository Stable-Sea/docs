...
For high-volume scenarios, you can create multiple payment instruments for different currencies:

```bash
# Create multiple payment instruments for different currencies
curl -X POST "https://sandbox.stablesea.com/terminal/v1/customers/cust_7f8b9c2d3e4f5a6b/external_payment_instruments" \
  -H "Authorization: Bearer YOUR_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
while [ $retry_count -lt $max_retries ]; do
  response=$(curl -w "%{http_code}" -X POST "https://sandbox.stablesea.com/terminal/v1/customers/cust_id/orders" \
    -H "Authorization: Bearer YOUR_API_KEY" \
    -H "Idempotency-Key: $(uuidgen)" \
    -d '{"quote_id":"quo_id"}')
