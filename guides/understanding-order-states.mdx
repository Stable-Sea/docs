...
The Stable Sea API uses **just-in-time (JIT) funding**, meaning organizations send payment **after** creating an order from a quote. This creates a time-sensitive window where several factors can impact success:
...
**Blockchain Congestion** is the most frequent cause of funding timeouts when organizations send payment after creating an order but blockchain delays prevent confirmation before the quote expires.

**Why this happens:**

- High network activity delays transaction confirmation
- Organization sets low gas fees to save money
- Network validators prioritize higher-fee transactions
- L1 networks (Ethereum) more susceptible than L2s (Solana)

Understanding this timing dynamic is crucial for setting proper organization expectations and choosing appropriate payment methods.

## Successful Order States

### STEP_1_AWAITING_FUNDING

**What's happening:** Order created, waiting for organization to send funds
...
- Organization should send payment to the provided address/details
...
- No organization action required
...
The FX conversion is done. Funds are being sent to the organization's destination payment instrument.

**What to expect:**

- Destination currency has been secured
- Payout is being processed to organization's account
...
Terminal success state. Funds have been delivered to the organization's destination account.

**What to expect:**

- Organization has received funds in their destination account
...
**Why this happens:** Organization funds arrive after the quote expiration time
...
- **Low gas fees** - Organization sets insufficient gas, causing slow processing
- **Organization delay** - Taking too long to send payment after order creation
- **Network timing** - Weekend/holiday periods when quotes may expire during off-hours

**What happens:**

- Order cannot be recovered or extended
- Organization must create a new quote and order to retry
- Original exchange rate is no longer available
...
- Organization funds are typically refunded automatically
- Rare occurrence under normal conditions
- May need to try different offering if recurring

### FAILED_REFUND_REQUESTED

**Why this happens:** Organization or system initiates a refund request

A refund has been requested before the order completed successfully.

**Common causes:**

- **Organization change of mind** - No longer wants the transaction
- **Error correction** - Organization realizes they made a mistake
- **Emergency situations** - Organization needs funds returned urgently
...
- Organization has received their refund
- Order is permanently closed
- Organization can create new order if still needed
...
- **FAILED_FUNDING_EXPIRED**: Organization must create new quote and order to retry
...
- **FAILED_REFUND_PROCESSED**: Organization has received refund, order closed
...
<Note>
  **Remember:** Order states reflect real-world payment and settlement
  processes. Understanding each state helps you set proper organization
  expectations and handle edge cases gracefully.
</Note>
...
Understanding these states helps you communicate clearly with organizations about where their order stands and what to expect next.
