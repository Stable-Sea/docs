...
<ResponseField name="funding_instructions.details" type="object">
  Payment method-specific funding details. For **NGN** orders (`currency` NGN, `method` NGN_BANK_ACCOUNT), this object contains the pay-in instructions the end user needs to send Nigerian Naira to a Nigerian bank: `bank_name`, `bank_account_number`, `beneficiary`, and `reference`. Display these to the user so they can complete the bank transfer; the user must include `reference` with the transfer. For other currencies (e.g. MXN, BRL), the structure differs; see the NGN pay-in flow guide for the full flow.
</ResponseField>
...
| -------- | ------ | -------------- |
| NGN | `NGN_BANK_ACCOUNT` | `bank_name`, `bank_account_number`, `beneficiary`, `reference` |
